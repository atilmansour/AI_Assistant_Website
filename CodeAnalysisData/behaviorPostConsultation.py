...
# CONFIG YOU WILL EDIT
# Name of File used when DATA_FORMAT = "FULL_JSON".
FULL_JSON_PATH = PROJECT_DIR / "exampleDataFiles" / "sessions_full.json"

# CONFIG YOU WILL EDIT
# Name of File used when DATA_FORMAT = "FULL_CSV".
...
def parse_json_field(value: Any, default: Any = None) -> Any:
    ...
    chat_events, and logs are saved as JSON strings. This helper turns those
    strings back into lists/dictionaries so the rest of the analysis can use them.
    """
    if default is None:
        default = None
...
def has_process_level_data(record: Dict[str, Any]) -> bool:
    """Return True when the record contains messages/editor data needed for this analysis."""
    normalized = normalize_downloaded_session(record, source_name=str(record.get("session_id", "record")))
    return bool(normalized.get("messages") or normalized.get("editor"))
...
def analyze_file(path: Path) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    normalized = normalize_downloaded_session(data, source_name=path.name)
    return analyze_session_data(normalized, source_name=path.name)
